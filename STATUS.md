...
**Last updated:** 2026-08-19 (**OBD-70 PID data logging — CODE-COMPLETE + REVIEW-APPROVED (3 rounds), `status: in-review`, awaiting Taras Garmin device-verify (HARD STOP for a drive test).** Branch `feat/70-pid-logging` in worktree `.claude/worktrees/obd70`; code head `25be53a`, review head `2534a41` (`reviews/OBD-70-round1.md`, `verdict: approved`, `reviewed-commit: 25be53a` == code head → staleness-clean). Gate green all rounds; `:app:test` demo 417 / prod 341, 0 failures. Feature: Record button in dashboard top row by the ⚙ → confirm dialog → 1 Hz **wide CSV** of the full mapped PID set (prod widens poll set to `PidCatalog.definitions` 18+ via per-flavor `LoggablePidsModule` DI — main/ never imports :core:protocol; reverts to GAUGE_CATALOG on stop via new `ActivePollSet` SSOT) written in `ObdConnectionService` (recorder inhibits OBD-69 idle-watchdog while recording) → `getExternalFilesDir("logs")` + `logs/index.json` → Recordings screen nested in SettingsRoute (list + FileProvider `ACTION_SEND` share + delete), USB-MTP fallback; all API-23-safe. Review chain: r1 changes-requested (blocker: recorder writer/field cross-thread races + no IO try/catch → mid-drive service crash) → r2 approved (lock discipline + graceful IOException finalize + CoroutineExceptionHandler + index Mutex + FileProvider fallback path + HHmmss filenames) → r3 confirm (fd/stray-file leak on storage-full-at-record-tap closed). **NOT MERGED** — `hardware-verify: true`, gated on Taras. Frontend-design skill NOT invoked (builder reused existing plain Material3 idiom — flagged to Taras as a design-polish decision, not a defect). **Garmin API-23 prod test build:** being produced now for the drive test (temp branch = garmin backport + feat/70). After device-verify: merge OBD-70 to main via merge.sh, then ONE `garmin-overlander-api23` rebase on new main (OBD-67+69+70). **Prior, still true:** OBD-69 idle battery-saver MERGED to main `a3977a2` + Garmin drive-VERIFIED (overnight drain gone; within-session reconnect fine). Main-channel demo APK `builds/main/app-main-a3977a2.apk`. **NOT pushed to GitHub** (main is 3 commits ahead of origin: OBD-70 spec + OBD-69 merge + status — awaiting Taras per-action OK). **OBD-71 filed** (`status: open`, deferred): overnight cold-reconnect needs a one-time quit+relaunch; likely OBD-69 idle-stop's flip side (nothing re-arms the service overnight); root-cause on device first, fix must NOT regress the OBD-69 drain.
