...
**Last updated:** 2026-08-18 (**OBD-69 idle battery-saver — MERGED to main `a3977a2`, Garmin drive-VERIFIED.** Taras confirmed on a real drive: overnight drain gone, engine-restart reconnects within-session, live drive not stalled (wake-lock refresh held). Hardware checklist in `issues/OBD-69.md`; merged via merge.sh, gate green both passes. Main-channel APK regenerated: `builds/main/app-main-a3977a2.apk` (demo-flavor). **NOT pushed to GitHub yet** — awaiting Taras per-action OK. **OBD-71 filed** (`status: open`) for the one follow-up Taras flagged: after sitting *overnight*, the app won't auto-reconnect on the next engine start without a one-time quit+relaunch (within-session reconnect is fine) — likely the OBD-69 idle-stop's expected-but-unwanted side (nothing re-arms the service overnight); root-cause on device first, fix must NOT regress the OBD-69 drain. Deferred by Taras. **NOW BUILDING: OBD-70 — PID data logging** (`issues/OBD-70.md`, filed): Record button by the gear → confirm dialog → 1 Hz wide-CSV of the full mapped PID set written in ObdConnectionService (poll-set widens while recording, reverts on stop) → `getExternalFilesDir("logs")` → Recordings screen under Settings (list + FileProvider share sheet + delete), USB-MTP folder as guaranteed fallback; all API-23-safe for the Garmin. `hardware-verify: true`. Garmin rebase deferred: do ONE `garmin-overlander-api23` rebase on new main after OBD-70 lands (both touch ObdConnectionService) rather than two. Temp `garmin-obd69-test` branch retired.
