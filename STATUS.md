...
**2026-09-12 — RECONNECT FIXES MERGED + GITHUB BACKUP.** OBD-71 (overnight cold-reconnect), OBD-78 (data-liveness wedge watchdog), OBD-82 (Connect restarts service) all squash-merged to main (gate GREEN pre+post each; both watchdogs launched in `ObdConnectionService.startWatchdogs`, both `MainActivity` fixes present; 71 test suites green). Garmin-verified by Taras (combined build). **main PUSHED to GitHub `27ccd06`** (origin was stale at `c6fb76f`/OBD-73 — 22 commits, ~2.5wk, now backed up; pushed via gh HTTPS token, SSH key unusable). ⚠️ `merge.sh`'s `git add -A` swept Taras's untracked personal `writing/` file into the OBD-71 squash — PURGED from all history via filter-branch before push (GitHub 404-confirms absent; file intact on disk; `writing/` now gitignored). Harden merge.sh staging later. **OBD-84** connection-status pill building; **OBD-83** immersive HELD pending Taras on-device confirm. garmin-conn-71-78-82 + feat/83 + feat/84 branches still local (unpushed).
...
| UX — OBD-84 (Connection-status pill) | +250k | 🔄 in progress 2026-09-12 | — | — |
