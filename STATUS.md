...
| Blockers | None. **RESIZABLE GRID feature in progress** (foundation-first, ~500k authorized): OBD-62 Phase 1 (grid model + repack engine + persistence, merged aa64afe) + OBD-63 Phase 2 (spanning grid renderer, merged b7a0a17) DONE — dashboard now renders as a spanning grid, on-device smoke passed on the Pixel (landscape 4-in-row, portrait 2×2, swap picker intact). NEXT: Phase 3 (OBD-64) = add/remove gauges + size-from-menu UI (>4 gauges, resize). Then reassess for gesture phases (resize handles / drag-drop). ⚠️ DEMO flavor shares prod appId — installing demo on the Pixel CLOBBERS the van build; always reinstall builds/van/app-prod-debug.apk after. |
