...
**Last updated:** 2026-08-17 (**OBD-69 idle battery-saver — CODE-COMPLETE + REVIEW-APPROVED, awaiting Taras device-verify (HARD STOP for a real drive test).** ObdConnectionService held a 12h PARTIAL_WAKE_LOCK → drained the Garmin overnight retrying an unreachable dongle. Fix on branch `feat/69-idle-battery-saver` (head `7690fe6`, status `in-review`): idle watchdog stops the service after 20 min of no data (existing teardown releases the wake lock), wake-lock backstop 12h→25 min AND now REFRESHED on each data sample (round-1 blocker — a fixed 25-min cap would've stalled a live screen-off drive >25 min; refresh means it lapses ~25 min after data STOPS). Pure `IdleWatchdog` (unit-tested) + Robolectric service tests incl. an airtight no-ref-count-leak test. Review approved at `73f6027` (`reviews/OBD-69-round1.md`, round 2); one reviewer-directed test tightening landed at `7690fe6` after — needs a quick re-confirm of that test-only delta at merge time. **NOT MERGED** — `hardware-verify: true`, gated on Taras: leave open overnight no-dongle → no drain (service self-stops ~20 min); reopen reconnects; long screen-off drive doesn't stall. **Combined Garmin build served** for that test: temp branch `garmin-obd69-test` (clean merge of garmin minSdk-23 backport + OBD-69) built a minSdk-23 APK with OBD-67+OBD-69, served at http://10.33.59.82:8000 (also builds/garmin/app-garmin-obd69-a547731.apk). After device-verify: merge OBD-69 to main, then rebase the real `garmin-overlander-api23` on new main for the permanent Garmin build.
